...
void setup_encoder_and_metadata(draco::PointCloud * point_cloud_or_mesh, draco::Encoder & encoder, int compression_level, int quantization_bits, float quantization_range,
  const float * quantization_origin, bool create_metadata, int rgba_len = 0, int normal_len = 0) {
  ...
  if (quantization_origin == NULL || quantization_range == -1) {
    ...
    if (rgba_len > 0){
      encoder.SetAttributeExplicitQuantization(draco::GeometryAttribute::COLOR, quantization_bits, rgba_len, quantization_origin, quantization_range);
    }
    if (normal_len > 0 ) {
      encoder.SetAttributeExplicitQuantization(draco::GeometryAttribute::NORMAL, quantization_bits, normal_len, quantization_origin, quantization_range);
    }
    if (create_metadata) {
...
EncodedObject encode_point_cloud(const std::vector < float > & points, const std::vector <float > & rgba, const std::vector <float > & normal,
  int rgba_len, int normal_len,
  int quantization_bits, int compression_level, float quantization_range,
  const float * quantization_origin, bool create_metadata
  ) {
  ...
  const int rgba_att_id = (rgba_len > 0) ? pcb.AddAttribute(draco::GeometryAttribute::COLOR, rgba_len, draco::DataType::DT_FLOAT32): -1;
  const int normal_att_id = (normal_len > 0) ? pcb.AddAttribute(draco::GeometryAttribute::NORMAL, normal_len, draco::DataType::DT_FLOAT32) : -1;
  ...
  draco::Encoder encoder;
  setup_encoder_and_metadata(point_cloud, encoder, compression_level, quantization_bits, quantization_range, quantization_origin, create_metadata, rgba_len, normal_len);
  draco::EncoderBuffer buffer;
